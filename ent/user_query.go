...
func (uq *UserQuery) QueryEvents() *EventsQuery {
	query := (&EventsClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithEvents(opts ...func(*EventsQuery)) *UserQuery {
	query := (&EventsClient{config: uq.config}).Query()
...
func (uq *UserQuery) loadEvents(ctx context.Context, query *EventsQuery, nodes []*User, init func(*User), assign func(*User, *Events)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int32]map[*User]struct{})
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(events.FieldID), joinT.C(user.EventsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Events](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
